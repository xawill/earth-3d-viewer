// Taken from https://github.com/NASA-AMMOS/3DTilesRendererJS/blob/master/example/src/plugins/overlays/TextureOverlayPlugin.js
...
export class TextureOverlayPlugin {
	...
	processTileModel(scene, tile) {
		...
		for (const key in caches) {
			const cache = caches[key];
			const pr = cache.loadTexture(this.getTileKey(tile)).catch(() => {});
	...
	registerLayer(name, customTextureCallback) {
		...
		if (this.waitForLoadCompletion) {
			...
			tiles.forEachLoadedModel((scene, tile) => {
				const pr = cache.loadTexture(this.getTileKey(tile)).then(() => {
					return { scene, tile };
				});
